...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		else
		{
			...
			if(parentNode == null) throw new TreeException("parent node not found");//??? If the parent node wasn't found, there must have been something wrong in the 
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("there is no node named " + name1);//??? Throw a TreeException with a useful message
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("there is no node named " + name2);//??? Throw TreeException with a useful message
